...
(function ($, obj) {
  ...
  obj.unbindMultidayEvents = function ($container) {
    var $hiddenMultidayEvents = $container.find(
      obj.selectors.hiddenMultidayEvent
    );
  ...
  obj.bindMultidayEvents = function ($container) {
    ...
    $hiddenMultidayEvents.each(function (hiddenIndex, hiddenMultidayEvent) {
      ...
      var $visibleMultidayEvents = obj.findVisibleMultidayEvents(
        $container,
        $hiddenMultidayEvent
      ); // eslint-disable-line max-len

      $visibleMultidayEvents.each(function (
        visibleIndex,
        visibleMultidayEvent
      ) {
        var $visibleMultidayEvent = $(visibleMultidayEvent);
        var $visiblemultidayEventBarInner = $visibleMultidayEvent.find(
          obj.selectors.multidayEventBarInner
        ); // eslint-disable-line max-len
  ...
  obj.initAllowedViews = function ($container) {
    ...
    $container.trigger("afterMultidayEventsInitAllowedViews.tribeEvents", [
      $container,
    ]);
  };
