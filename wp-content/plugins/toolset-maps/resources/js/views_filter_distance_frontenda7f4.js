...
WPViews.DistanceFilterFrontend = function ($) {
  ...
  self.hiddenCoordsForm =
    '<form id="js-toolset-maps-coords-form" class="js-wpv-ajax-results-enabled"></form>';
  ...
  self.eventsOn = function () {
    ...
    $(document).on(
      "toolset_maps_views_distance_filter_center_updated",
      function () {
        ...
        if (
          $(self.distanceCenterSelector)
            .parents("form")
            .hasClass("js-wpv-ajax-results-enabled")
        ) {
  ...
  self.init = function () {
    ...
    if (navigator.geolocation && self.isSecurePage()) {
      ...
      $(document).on(
        "toolset_maps_views_distance_filter_user_location_acquired",
        function () {
      ...
      self.acquireCurrentLocation(
        function (data) {
          ...
          var queryResultsPromise =
            WPViews.view_frontend_utils.get_updated_query_results(
              self.view_id,
              0,
          ...
          queryResultsPromise.done(function (data, textStatus, jqXHR) {
            ...
            $('form[name^="wpv-filter-' + data.data.id + '"').replaceWith(
              data.data.full
            );
        ...
        function (error) {
          window.alert(
            toolset_maps_distance_filter_settings.geolocation_error +
              error.message
          );
        }
      );
