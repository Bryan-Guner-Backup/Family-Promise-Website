...
WPViews.ViewAddonMaps = function ($) {
  ...
  self.waitForMapInitThenInitStreetView = function (mapId, $marker) {
    $(document).on(
      "js_event_wpv_addon_maps_init_map_completed",
      function (event, map) {
        if (mapId === map.map_id) {
          var latlon = new google.maps.LatLng(
            $marker.data("markerlat"),
            $marker.data("markerlon")
          );
  ...
  self.collect_map_data = function (thiz_map) {
    ...
    $(".js-wpv-addon-maps-markerfor-" + thiz_map_id).each(function () {
      ...
      if (
        ...
        if (thiz_marker.data("markerlon") === "wait") {
          ...
          self.add_current_visitor_location_after_geolocation(
            thiz_map_id,
            thiz_marker
          );
        } else {
          self.add_current_visitor_location_after_init(
            thiz_map_id,
            thiz_marker
          );
        }
    ...
    if (
      (thiz_map.data("location") === "first" || thiz_map.data("markerid")) &&
      !streetViewMarkerFound
    ) {
    ...
    thiz_map_options["show_layer_interests"] =
      thiz_map.data("showlayerinterests");
    ...
    thiz_map_options["full_screen_control"] =
      thiz_map.data("fullscreencontrol");
    thiz_map_options["zoom_control"] = thiz_map.data("zoomcontrol");
    thiz_map_options["street_view_control"] =
      thiz_map.data("streetviewcontrol");
    ...
    var thiz_cluster_options = {
      ...
      maxZoom:
        parseInt(thiz_map.data("clustermaxzoom")) > 0
          ? parseInt(thiz_map.data("clustermaxzoom"))
          : null,
    ...
    if (_.has(self.has_cluster_options, thiz_map_id)) {
      if (_.has(self.has_cluster_options[thiz_map_id], "styles")) {
        thiz_cluster_options["styles"] =
          self.has_cluster_options[thiz_map_id]["styles"];
      }
      if (_.has(self.has_cluster_options[thiz_map_id], "calculator")) {
        thiz_cluster_options["calculator"] =
          self.has_cluster_options[thiz_map_id]["calculator"];
      }
    }
   ...
   * @param {jQuery} thiz_marker
   */
  self.add_current_visitor_location_after_init = function (
    thiz_map_id,
    thiz_marker
  ) {
    $(document).on(
      "js_event_wpv_addon_maps_init_map_completed." + thiz_map_id,
      function (event, data) {
   ...
   * @param {jQuery} thiz_marker
   */
  self.add_current_visitor_location_after_geolocation = function (
    thiz_map_id,
    thiz_marker
  ) {
    $(document).one(
      "js_event_wpv_addon_maps_map_data_collected." + thiz_map_id,
      function () {
        self.add_current_visitor_location(thiz_map_id, thiz_marker);
      }
    );
   ...
   * @since 1.4.2
   */
  self.check_being_inside_collapsed_bootstrap_accordion = function (
    map_selector
  ) {
    var accordion_id = "";
    var toggle_href = "";
  ...
  self.initMapOrWaitIfInsideHiddenBootstrapAccordionOrTab = function (map) {
    ...
    var accordion_id =
      self.check_being_inside_collapsed_bootstrap_accordion(map_selector);
  ...
  self.init_maps = function () {
    self.maps_data.map(function (map) {
      ...
      if (map.options.render && map.options.render === "wait") {
        return true;
      }
  ...
  self.init_map = function (map) {
    ...
    $(document).trigger("js_event_wpv_addon_maps_init_map_started", [
      event_settings,
    ]);
    ...
    const mapElement = document.getElementById(
      "js-wpv-addon-maps-render-" + map.map
    );
    ...
    $(document).trigger("js_event_wpv_addon_maps_init_map_inited", [
      event_settings,
    ]);
    ...
    map.markers.map(function (marker) {
      var marker_lat_long = new self.api.LatLng(
          marker.markerlat,
          marker.markerlon
        ),
        marker_map_icon =
          marker.markericon == "" ? map_icon : marker.markericon,
        marker_map_icon_hover =
          marker.markericonhover == ""
            ? map_icon_hover
            : marker.markericonhover,
        marker_settings = {
      ...
      self.markers[map.map][marker.marker] = new self.api.Marker(
        marker_settings
      );

      self.bounds[map.map].extend(
        self.markers[map.map][marker.marker].position
      );

      if (marker_map_icon != "" || marker_map_icon_hover != "") {
        marker_map_icon =
          marker_map_icon == ""
            ? views_addon_maps_i10n.marker_default_url
            : marker_map_icon;
        marker_map_icon_hover =
        ...
        if (marker_map_icon != marker_map_icon_hover) {
          ...
          self.api.event.addListener(
            self.markers[map.map][marker.marker],
            "mouseover",
            function () {
              self.markers[map.map][marker.marker].setIcon(
                marker_hover_icon_scaled
              );
            }
          );
          self.api.event.addListener(
      ...
      if (marker.markerinfowindow !== "") {
        ...
        // the marker
        self.infowindows[map.map] =
          self.infowindows[map.map] || new self.api.InfoWindow({ content: "" });
        self.api.event.addListener(
          self.markers[map.map][marker.marker],
          clickEvent,
          function () {
            self.infowindows[map.map].setContent(marker.markerinfowindow);
            self.infowindows[map.map].open(
              self.maps[map.map],
              self.markers[map.map][marker.marker]
            );
          }
        ...
        $(document).on(
          "click",
          ".js-toolset-maps-open-infowindow-map-" +
            map.map +
            "-marker-" +
            marker.marker,
          function () {
    ...
    if (_.size(map.markers) == 1) {
      if (map.options["single_zoom"] != "") {
        ...
        if (map.options["fitbounds"] == "on") {
          self.api.event.addListenerOnce(
            self.maps[map.map],
            "bounds_changed",
            function (event) {
              self.maps[map.map].setZoom(map.options["single_zoom"]);
      ...
      if (map.options["single_center"] == "on") {
        for (var mark in self.markers[map.map]) {
          self.maps[map.map].setCenter(
            self.markers[map.map][mark].getPosition()
          );
          break;
        }
      }
    ...
    if (map.options.lat && map.options.long) {
      self.initStreetView(
        event_settings,
        new google.maps.LatLng(map.options.lat, map.options.long)
      );
    }
  };
  /**
  ...
  self.openInfowindowWhenMarkerVisible = function (mapId, markerId) {
    if (self.markers[mapId][markerId].map) {
      self.infowindows[mapId].open(
        self.maps[mapId],
        self.markers[mapId][markerId]
      );
    } else {
      _.delay(function () {
  ...
  self.clean_map_data = function (map_id) {
    ...
    $(document).trigger("js_event_wpv_addon_maps_clean_map_completed", [
      settings,
    ]);
  };
  ...
  self.initMapsAzure = function () {
    let maps = _.map(
      self.maps_data,
      self.resolveGeolocatedMarkerThenInitMapAzure
    );
    ...
    let pins = _.map(map.markers, function (marker) {
      return new atlas.data.Feature(
        new atlas.data.Point([marker.markerlon, marker.markerlat]),
        {
          ...
          iconhover: marker.markericonhover
            ? marker.markericonhover
            : map.options.marker_icon_hover,
          id: marker.marker,
        },
        marker.marker
      );
  ...
  self.reload_map = function (map_id) {
    ...
    $(document).trigger("js_event_wpv_addon_maps_reload_map_started", [
      settings,
    ]);
    $(document).trigger("js_event_wpv_addon_maps_reload_map_triggered", [
      settings,
    ]);
    $(document).trigger("js_event_wpv_addon_maps_reload_map_completed", [
      settings,
    ]);
  };
   ...
   */
  $(document).on(
    "js_event_wpv_addon_maps_reload_map_triggered",
    function (event, data) {
      var defaults = {
          map_id: false,
        },
      ...
      var mpdata = self.collect_map_data(
        $("#js-wpv-addon-maps-render-" + settings.map_id)
      );
      ...
      if (API_GOOGLE === views_addon_maps_i10n.api_used) {
        ...
        self.maps[settings.map_id] =
          self.resolveGeolocatedMarkerThenInitMapAzure(mpdata);
      }
    }
  );
  ...
  self.get_map_marker = function (marker_id, map_id) {
    if (map_id in self.markers && marker_id in self.markers[map_id]) {
  ...
  $(document).on("click", ".js-wpv-addon-maps-focus-map", function (e) {
    ...
    var thiz = $(this),
      ...
      thiz_marker,
      thiz_zoom;
    if (thiz.attr("data-map") && thiz.attr("data-marker")) {
      ...
      if (API_GOOGLE === views_addon_maps_i10n.api_used) {
        if (
          ...
          thiz_zoom =
            $("#js-wpv-addon-maps-render-" + thiz_map).data("singlezoom") != ""
              ? $("#js-wpv-addon-maps-render-" + thiz_map).data("singlezoom")
              : 14;
          self.maps[thiz_map].setCenter(
            self.markers[thiz_map][thiz_marker].getPosition()
          );
        ...
        if (thiz_map in self.maps) {
          ...
          self.maps[thiz_map].setCamera({
            center: [
              $thisMarker.eq(0).data("markerlon"),
              $thisMarker.eq(0).data("markerlat"),
            ],
            zoom: thiz_zoom,
          });
        }
  ...
  $(document).on("click", ".js-wpv-addon-maps-restore-map", function (e) {
    ...
    if (thiz.attr("data-map")) {
      thiz_map = thiz.data("map");
      if (thiz_map in self.maps && thiz_map in self.bounds) {
        self.maps[thiz_map].fitBounds(self.bounds[thiz_map]);
      }

      if (_.size(self.markers[thiz_map]) == 1) {
        current_map_data_array = _.filter(
          self.maps_data,
          function (map_data_unique) {
        ...
        if (current_map_data.options["single_zoom"] != "") {
          ...
          if (current_map_data.options["fitbounds"] == "on") {
            self.api.event.addListenerOnce(
              ...
              "bounds_changed",
              function (event) {
                self.maps[thiz_map].setZoom(
                  current_map_data.options["single_zoom"]
                );
              }
            );
          }
        ...
        if (current_map_data.options["single_center"] == "on") {
          for (var mark in self.markers[thiz_map]) {
            self.maps[thiz_map].setCenter(
              self.markers[thiz_map][mark].getPosition()
            );
            break;
          }
        }
  ...
  self.cleanupMarkersFromMapBlock = function (viewUniqueId) {
    const viewId = _.isNumber(viewUniqueId)
      ? viewUniqueId
      : viewUniqueId.split("-")[0];
    ...
    $(".js-wpv-addon-maps-marker").each(function (key, marker) {
      ...
      if (Number($(marker).data("fromview")) === Number(viewId)) {
        ...
        self.view_map_blocks_relation[viewUniqueId] =
          $(marker).data("markerfor");
  ...
  self.get_affected_maps = function (container) {
    ...
    if (self.view_map_blocks_relation[container.data("viewnumber")]) {
      affected_maps.push(
        self.view_map_blocks_relation[container.data("viewnumber")]
      );
    }
  ...
  self.get_cluster_options = function (map_id) {
    ...
    if (typeof map_id !== "undefined" && _.has(self.cluster_options, map_id)) {
  ...
  self.initGoogle = function () {
    if (self.api === null) {
      ...
      // Lazy loading failed, don't get into an infinite loop.
      return;
    }
    ...
    $(document).on(
      ...
      function (event) {
        $(".js-wpv-layout-responsive .js-wpv-addon-maps-render").each(
          function () {
            var thiz = $(this),
              thiz_map = thiz.data("map");
            if (thiz_map in self.maps) {
  ...
  self.initMapById = function (id) {
    ...
    const mapData = self.collect_map_data(
      $("div#js-wpv-addon-maps-render-" + id)
    );
    ...
    if (self.api === null) {
      $.getScript(
        ...
        .fail(function () {
          self.api = false;
          console.warn(
            "Toolset Maps: failed to lazy load Google Maps library."
          );
        });
    } else {
      initMap(mapData);
